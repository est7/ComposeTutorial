...
sealed class ListLoadState(
    ...
    companion object {
        fun isRefreshing(state: ListLoadState) = state.type == LoadType.REFRESH && state.result == LoadResult.NONE
        fun isRefreshSuccess(state: ListLoadState) =
            state.type == LoadType.REFRESH && state.result == LoadResult.SUCCESS

        fun isRefreshFailed(state: ListLoadState) = state.type == LoadType.REFRESH && state.result == LoadResult.FAILED
        fun isRefreshNoMoreData(state: ListLoadState) =
            state.type == LoadType.REFRESH && state.result == LoadResult.NO_MORE_DATA


        fun isLoadingMore(state: ListLoadState) = state.type == LoadType.LOAD_MORE && state.result == LoadResult.NONE
